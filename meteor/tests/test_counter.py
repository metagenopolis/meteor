...
def test_save_cram(counter_unique: Counter, datadir: Path, tmp_path: Path) -> None:
    ...
    with AlignmentFile(str(cramfile.resolve()), "rc") as cramdesc:
        reads, _ = counter_unique.filter_alignments(cramdesc)
        raw_cram_header = cramdesc.header  # pylint: disable=unused-variable
    merged_list = heapq.merge(*(reads.values()))
    tmpcramfile = tmp_path / "test"
    counter_unique.save_cram_strain(
        cramfile.resolve(), tmpcramfile, raw_cram_header, merged_list, ref_json
    )
...
def test_launch_counting_unique(
