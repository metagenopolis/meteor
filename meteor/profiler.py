...
class Profiler(Session):
    ...
    def __post_init__(self):
        ...
        try:
            ...
            sys.exit()
        for db in list(self.annot_db_dict.values()) + list(
            self.module_db_dict.values()
        ):
            self.check_file(db, {self.meteor.gene_column, self.meteor.ko_column})

        # Define output names
        gene_table_output = (
            self.meteor.mapping_dir / f"{self.sample_name}_{self.suffix_file}_norm.tsv"
        )
        msp_table_output = (
            self.meteor.mapping_dir / f"{self.sample_name}_{self.suffix_file}_msp.tsv"
        )
        functions_table_output = {
            db: self.meteor.mapping_dir
            / f"{self.sample_name}_{self.suffix_file}_{db}_functions.tsv"
            for db in self.annot_db_list
        }
        modules_table_output = (
            self.meteor.mapping_dir
            / f"{self.sample_name}_{self.suffix_file}_modules.tsv"
        )
    ...
    def normalize_fpkm(self, rarefaction_level: int, unmapped_reads: int) -> None:
        ...
        if rarefaction_level > 0:
            unmapped_reads_after_rf = (
                self.rarefaction_level - self.gene_count[count_column].sum()
            )
        else:
        ...
        self.gene_count[count_column] = self.gene_count[count_column].div(
    ...
    def compute_ko_abundance_by_msp(
        ...
        ko_dict_ab = {
            ko: self.msp_table.loc[
                self.msp_table[self.meteor.msp_column].isin(msp_set),
                self.meteor.value_column,
            ].sum()
            for (ko, msp_set) in ko_dict.items()
        }
        self.functions = (
            pd.DataFrame.from_dict(
    ...
    def execute(self) -> bool:
        ...
        self.gene_count.to_csv(
            self.output_filenames["gene_table_norm"], sep="\t", index=False
        )
        # Update config file
        config_norm = {}
        ...
        self.save_config(
            self.sample_config,
            Path(self.output_filenames["gene_table_norm"]).with_suffix(".ini"),
        )
        ...
        if self.compute_msp_bool or self.by_msp:
            if not self.compute_msp_bool:
                logging.info(
                    "MSP abundances will be computed since you requested to compute functions via MSP."
                )
            ...
            self.msp_table.to_csv(
                self.output_filenames["msp_table"], sep="\t", index=False
            )
            # Compute MSP stats
            ...
            self.save_config(
                self.sample_config,
                Path(self.output_filenames["msp_table"]).with_suffix(".ini"),
            )
            ...
            for db, annot_file in self.annot_db_dict.items():
                ...
                if self.by_msp:
                    ...
                    self.compute_ko_abundance_by_msp(
                        annot_file=annot_file, msp_def_filename=self.msp_filename
                    )
                else:
                    logging.info(
                        "MSP abundance are not used. Add --by_msp to use them for computations."
                    )
                    self.compute_ko_abundance(annot_file=annot_file)
                logging.info("Save annotation file.")
                self.functions.to_csv(
                    self.output_filenames["functions_table"][db], sep="\t", index=False
                )
                ...
                self.save_config(
                    update_config,
                    Path(self.output_filenames["functions_table"][db]).with_suffix(
                        ".ini"
                    ),
                )
        ...
        if self.compute_modules_bool:
            ...
            self.mod_table.to_csv(
                self.output_filenames["modules_table"], sep="\t", index=False
            )
            # Update config files
